...
class Shop {
    constructor() {
        this.items = [
            ...
            new Item('<i class="fas fa-book"></i> JavaScript: The Definitive Guide', 'bookJSDefinitiveGuide', [
    ...
    buyItem(e, player) {
        ...
        const item = this.items.filter((_item) => _item.id === target.dataset.id)[0];
    ...
    <a class="panel-block itemChoice" id="${item.id}Block" data-ID="${item.id}">
      ...
      ${itemMarkup}
    `);
        document.querySelectorAll('.itemChoice').forEach((itemEl) => {
            if (!(itemEl instanceof HTMLElement))
                return;
            itemEl.onclick = (e) => this.buyItem(e, player);
        });
    }
    loadShop(player) {
