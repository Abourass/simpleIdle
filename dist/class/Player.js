...
export default class Player {
    ...
    loadCareer(career) { this.careers = careers().load(career); }
    addItem(item) {
    ...
    update(prop, operation, amount) {
        ...
        if (document.getElementById(prop))
            el(`#${prop}`).text(this[prop]);
    }
    increaseStat(stat, fn) {
        if (this.statPoints > 0) {
            this.statPoints -= 1;
            if (document.getElementById('statPoints'))
                el('#statPoints').text(this.statPoints);
            this[stat] += 1;
            if (document.getElementById(stat))
                el(`#${stat}`).text(this[stat]);
            if (this.statPoints === 0) {
                fn();
                console.log('Called CB FN()');
            }
        }
    }
    addJobExp(amount) {
        if (this.careers.currentPath === 'none') {
            return;
        }
        this.careers.currentPath.addExp(amount, this);
    }
    init() {
        const initTypes = ['int', 'dex', 'char', 'perc', 'money', 'statPoints', 'health'];
